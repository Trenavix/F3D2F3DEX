...
public class ROM
{
    ...
    private static uint[] Segments = new uint[0x21]; //Max 0x1F RAM segments in ROM
    private static uint[] BackupSegments = new uint[0x21];
    ...
    public uint readSegmentAddr(UInt32 SegAddr)
    {
        ...
        if (segment > 0x1F) throw new Exception(SegAddr.ToString("x"));
        return SegmentAddrtoROMAddr(segment, addr);
    }

    public void BackupAllSegments()
    {
        Array.Copy(Segments, BackupSegments, Segments.Length);
    }

    public void LoadBackupSegments()
    {
        Array.Copy(BackupSegments, Segments, BackupSegments.Length);
    }
